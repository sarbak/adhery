    ...
    <div className="bg-white border border-border-light">
      ...
      <div className="p-4 border-b border-border-light flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-accent/10 text-accent flex items-center justify-center text-sm font-bold">
        ...
        <button onClick={onClose} className="text-text-muted hover:text-foreground text-xs">
          Close &times;
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 border-b border-border-light">
        {[
          ...
          <div key={s.label} className="p-3 border-r border-border-light last:border-r-0">
      ...
      <div className="p-4">
        <p className="text-xs font-medium text-foreground mb-3">Interaction Timeline</p>
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {patientInteractions.map((int) => (
            <div key={int.id} className="flex items-start gap-3 p-2 hover:bg-surface-warm transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-text-secondary mt-0.5 line-clamp-2">{int.content}</p>
                {int.metadata?.sentiment !== undefined && (
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-text-muted">Sentiment:</span>
                    <div className="w-16 h-1.5 bg-surface-warm">
                      <div
                    ...
                    <span className="text-[10px] text-text-muted">{Math.round(int.metadata.sentiment * 100)}%</span>
                  </div>
                )}
                {int.metadata?.duration && (
                  <span className="text-[10px] text-text-muted mt-0.5 block">Duration: {int.metadata.duration}</span>
                )}
              </div>
              <span className="text-[10px] text-text-muted flex-shrink-0">{int.createdAt}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-text-muted">{patients.length} enrolled patients</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <input
          ...
          className="bg-white border border-border-light px-3 py-2 text-xs text-foreground placeholder:text-text-muted outline-none focus:border-accent w-64"
        />
        <select
          value={statusFilter}
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-4">
        ...
        <div className="bg-white border border-border-light">
          {/* Table header */}
          <div className="grid grid-cols-[1fr_80px_80px_80px_90px_90px] px-4 py-2 border-b border-border-light text-[10px] text-text-muted uppercase tracking-wider">
            <span>Patient</span>
            <span>Status</span>
            <span>Adherence</span>
            <span>Risk</span>
          ...
          <div className="max-h-[600px] overflow-y-auto">
            {filtered.map((p) => (
              <div
                ...
                className={`grid grid-cols-[1fr_80px_80px_80px_90px_90px] px-4 py-2.5 border-b border-border-light cursor-pointer transition-colors ${
                  selectedPatient === p.id ? 'bg-accent/5 border-l-2 border-l-accent' : 'hover:bg-surface-warm border-l-2 border-l-transparent'
                }`}
              >
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 bg-accent/10 text-accent flex items-center justify-center text-[10px] font-bold flex-shrink-0">
                ...
                <span className={`text-xs self-center ${p.riskScore > 70 ? 'text-red-400' : p.riskScore > 40 ? 'text-yellow-400' : 'text-text-secondary'}`}>
                  {p.riskScore}
                </span>
                <span className="text-[10px] text-text-muted self-center">{p.lastContact}</span>
                <span className="text-[10px] text-text-muted self-center capitalize">{p.channelPreference}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {selectedPatient ? (
          ...
          <div className="bg-white border border-border-light flex items-center justify-center p-8">
            <p className="text-xs text-text-muted">Select a patient to view details</p>
          </div>
        )}
      </div>
    </div>
